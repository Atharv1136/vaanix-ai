...
export function TestCallWidget({ assistantId }: TestCallWidgetProps) {
  ...
  const subscribeToTranscripts = async () => {
    ...
    if (recentCalls && recentCalls.length > 0) {
      ...
      supabase
        ...
        .on(
          'postgres_changes',
          { event: 'INSERT', schema: 'public', table: 'call_transcripts', filter: `call_id=eq.${currentCallRecordId.current}` },
          (payload) => {
  ...
  return (
    <div className="flex flex-col h-[600px] w-full max-w-sm rounded-xl border border-border bg-card shadow-xl overflow-hidden sticky top-8">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {transcripts.length === 0 && status !== 'live' ? (
          <div className="h-full flex flex-col items-center justify-center text-muted-foreground space-y-4">
            ...
            <p className="text-sm text-center">Click the call button to start a<br/>browser-based test call.</p>
          </div>
        ) : (
          transcripts.map((msg, i) => (
            <div key={i} className={`flex flex-col ${msg.speaker === 'caller' ? 'items-end' : 'items-start'}`}>
              <span className="text-[10px] text-muted-foreground mb-1 uppercase tracking-wider">{msg.speaker}</span>
      ...
      {/* Controls */}
      <div className="p-4 border-t border-border bg-background/50 flex justify-center items-center">
        <button
